...
-   **Variable Selection**: Can visualize different accessibility metrics:

    -   Mean drive times (in seconds or minutes)
    -   Mean drive distances (in kilometers)
    -   Address counts (population coverage)

-   **Geographic Focus**: Filters data to include only the municipalities of interest (`CSD_NAMES`)

-   **Map Titles and Legends**: Customizable through function parameters

-   Uses the predefined `MAP_THEME` from settings.R for consistent visual appearance

-   Applies the `FILL_THEME` for continuous color scales

-   Includes Service BC location markers for geographic reference
...
#### Parameterization and Styling

-   **Smoothing Parameters**:
    -   Kernel bandwidth (sigma = 1000)
    -   Grid resolution (300x300 cells)
    -   Clipping to municipality boundaries
-   **Common Scale Option**: Toggle for consistent color scale across all maps
    -   When enabled, ensures direct visual comparability between municipalities
    -   When disabled, optimizes color range for each individual municipality
-   Uses the predefined `MAP_THEME` from settings.R for consistent visual appearance
-   Applies the `FILL_THEME` for continuous color scales
-   Includes Service BC location markers for geographic reference
...
The `02b-plot-localities.R` script creates a map that highlights the pilot municipalities and plots the locations of Service BC offices, providing a high-level overview of the geographic distribution of the selected localities.

#### Required Files

-   **BC Boundary**: Provincial outline from the `bcmaps` package
-   **CSD Shapefiles**: Processed Census Subdivision boundaries (`SHAPEFILE_OUT/processed_csd_with_location.gpkg`)
-   **Location Data**: Service BC office coordinates (`SRC_DATA_FOLDER/reduced-service_bc_locs.csv`)

#### Parameterization and Styling

-   **Label Placement**: Calculates and adjusts centroids for municipality labels to avoid overlap
-   **CSD Selection**: Automatically includes all municipalities defined in `CSD_NAMES`
-   Uses standard ggplot styling with customized colors and label formatting
-   Represents Service BC offices as yellow markers on the map
-   Includes a simple legend identifying Service BC locations

#### Visualization Specifics

-   **Geographic Context**: Shows the location of pilot municipalities within the broader context of British Columbia
-   **Output Format**: Saves a single map as `pilot_regions.png` in the `MAP_OUT` folder
...
-   **Variable Selection**: Can visualize any numeric metric in the dataset:
    -   Mean drive time (minutes)
    -   Mean drive distance (kilometers)
    -   Other statistical measures (variance, skewness, etc.)
-   **Plot Titles and Labels**: Customizable through function parameters
-   **Plot Types**: Configurable to generate box plots, violin plots, or both
-   Uses specialized theme constants from `settings.R`:
    -   `BOX_PLOT_THEME`: Optimized for box plots with rotated axis labels
    -   `VIOLIN_PLOT_THEME`: Adapted for violin plots
    -   `FILL_THEME_D`: Discrete color scale for distinguishing municipalities
