...
**Required files:** the aggregated statistics from the `01-descriptive-tables.R` script, primarily at the dissemination block level for greatest detail. This allows for customization to any metric present in the dataset such as mean drive time (minutes), mean drive distance (kilometers) and population density.

**Box Plots**: Show the central tendency, dispersion, and potential outliers in drive time/distance distributions.
...
-   Implemented through specialized function `build_boxplot()`

**Violin Plots**: Combine box plot characteristics with kernel density estimation to show the full distribution shape.
...
-   Implemented through specialized function `build_violinplot()`
