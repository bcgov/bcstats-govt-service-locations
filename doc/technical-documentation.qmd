...
-   `full-processed-drivetime-data.csv`: Processed drivetime data for all municipalities
-   `reduced-drivetime-data.csv`: Processed drivetime data for the municipalities of interest
-   `full-service-bc-locs.csv`: Service BC office locations for all municipalities
-   `reduced-service_bc_locs.csv`: Service BC office locations for the municipalities of interest
-   `csd-da-db-loc-correspondance.csv`: Correspondence file linking DBs, DAs, and CSDs in BC
-   `full-population-db.csv`: Census 2021 population data for all DBs
-   `reduced-population-db.csv`: Census 2021 population data for the DB's in municipalities of interest
-   `full-population-csd.csv`: Census 2021 population data for all CSDs
-   `reduced-population-csd.csv`: Census 2021 population data for CSD's involving the municipalities of interest
-   `full-population-projections.csv`: raw DB-level projections
-   `full_db_projections_transformed.rds`: DB-level projections after some initial transformation
-   `full-csd_with_location.gpkg`: Shapefiles for each CSD in British Columbia
-   `reduced-csd-with-location.gpkg`: Shapefiles for each CSD for the municipalities of interest
-   `full-db_with_location.gpkg`:  Shapefiles for each DB in British Columbia
-   `reduced-db-with-location.gpkg`: Shapefiles for each DB for the municipalities of interest
...
-   The script focuses on drive distance (`drv_dist`) as the primary metric, but can be adapted for drivetime or other variables.
...
* Ingests spatial coordinates for existing SBC facilities and a specific pilot location, executing a coordinate transformation from WGS 84 (EPSG:4326) to the BC Albers projection (EPSG:3005).
* Loads Dissemination Block (DB) geometries, Census Subdivision (CSD) crosswalks, and Statistics Canada population center boundaries to establish the spatial baseline.
* Accesses BC Stats population projections (RDS format) to integrate longitudinal demographic data for the years 2025, 2030, and 2035.
...
#### Data Preparation and Inputs
* Retrieves Dissemination Block geometries and provincial population projections via the BCDC API (Resource ID: `0e15d04d-127c-457a-b999-20800c929927`).
* Extracts 2021 Census data (CA21) through the `cancensus` API to obtain DB-level population and dwelling counts.
* Transforms facility point data from geographic coordinates (EPSG:4326) into the BC Albers projection (EPSG:3005) to ensure spatial alignment with provincial layers.
* Joins an external rural-urban classification matrix to provide CSD-level jurisdictional labels derived from the Ministry of Jobs and Economic Growth.
