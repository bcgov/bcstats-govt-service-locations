...
Global constants and configuration settings used throughout the Service BC data pipeline are centralized in a `settings.R` file. This file defines key parameters, file paths, and data validation requirements, ensuring consistency and maintainability across all pipeline components. Below is a summary of the most critical settings; for a complete list, refer to the `settings.R` file.
...
The following outputs are generated by the Service BC data pipeline:

1. **Dissemination Block Urban/Rural Classification**:
   - **File**: `rurality-by-db-{date}.csv`
...
2. **Service BC Facility Statistics**:
   - **File**: `sbc-location-statistics-for-SBC-{date}.csv`
