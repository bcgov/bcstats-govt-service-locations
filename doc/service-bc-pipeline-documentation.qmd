...
This document is a technical reference for the Service BC data pipeline, focusing on the core steps and methodologies from processing and handling of raw data through to aggregated output tables. The pipeline analyzes accessibility to Service BC offices across British Columbia.
...
::: {.callout-note collapse="true" icon="false"}
## Census Subdivisions and Dissemination Blocks Boundary Files
...
**Description**: Spatial boundary files for administrative and statistical geographies
  ...
  - **Hierarchical Structure**: Contains geographic identifiers linking DB → CSD
:::
...
::: {.callout-note collapse="true" icon="false"}
## Population Center Boundary Files

**Source**: Statistics Canada  
**Access Method**: Direct file download from Statistics Canada

  - Geographic files and documentation: `92-166-X`
  - Issue number: `2021001`

**Description**: Spatial boundary files for census geographies

  - **Coordinate System**: latitude and longitude coordinates based on North American Datum of 1983 (NAD83)
  - **Hierarchical Structure**: Contains geographic identifiers linking DB → CSD
...
Global constants and configuration settings used throughout the Service BC data pipeline are centralized in a `settings.R` file. This file defines key parameters, file paths, and data validation requirements and is required to run the pipeline scripts. Below is a summary of the most critical settings.  For a complete list, please refer to the `settings.R` file in the project [Github Repository](https://github.com/bcgov/bcstats-govt-service-locations/tree/main).

### Core Analysis Parameters

The following constants and parameters are essential for the Service BC data pipeline:

::: {.callout-note collapse="true" icon="false"}
## Data Processing and Filtering

  - `CENSUS_BASIS`: 2021 - Reference year for Census data.
  - `CURRENT_YEAR`: 2025 - Current year for projections and analysis.
  - `FACILITY_TAG`: "servicebc" - Tag used to identify Service BC facilities.
  - `REQUIRED_COLS`: Columns required for data validation:
    - `site_albers_x`, `site_albers_y`, `dissemination_block_id`, `drv_time_sec`, `drv_dist`, `tag`.
  - `POP_COLS`: Columns related to population data:
    - `region_name`, `area_sq_km`, `population`, `dwellings`, `households`.
...
## File Path Configuration
  ...
  - `LAN_FOLDER`: Base folder for network storage.
  - `SRC_DATA_FOLDER`: Directory for source data files.
  - `RAW_DATA_FOLDER`: Directory for raw input data.
  - `DT_DATA_FOLDER`: Directory for drivetime data.
  - `SBCLOC_FILEPATH`: Filepath for Service BC location data.
  - `SHAPEFILE_OUT`: Directory for shapefile outputs.
  - `TABLES_OUT`: Directory for output tables.
  - `VISUALS_OUT`: Directory for visualizations.
  - `FOR_SBC_OUT`: Directory for Service BC-specific outputs.
:::
...
Transform raw external data sources into standardized intermediate files for analysis. This is the foundation layer, creating all necessary data artifacts for downstream processing.
...
Analyze urban and rural accessibility patterns by classifying addresses using multiple methodological approaches. This steps provides geographic context for service accessibility and supports rural service delivery policy development.
...
Generate final output tables aggregated at the Service BC facility catchment level, incorporating drive time statistics, population data, and demographic projections for operational planning and facility-based analysis. Produce standardized CSV output focused on individual facility service areas.
...
This step utilizes intermediate datasets created by earlier pipeline processes (see [Data Preparation](#data-preparation) and [Catchment Assignment](#catchment-assignment) sections).  Dissemination Block (DB) and Population Center (POPCTR) geograpahic boundary files area also required.
...
- **Population Center Boundary Files**: `popcenter-statscan.gpkg` - Population Center boundaries from Statistics Canada
- **Dissemination Block Boundary Files**: `full-db-with-location.gpkg` - Dissemination Block boundaries from cancensus API
...
- `estimated_population_2025`, `5_yr_projection_2030`, `10_year_projection_2035` - Estimated population projections aggregated from assigned DBs.
...
- `n_x_y_km`, `n_180_plus_km` - Estimated number of residents within each distance bin.
- `n_within_x_y_min`,`n_over_150_min` - Estimated number of residents within each drivetime bin.
...
The Service BC data pipeline generates many interim outputs; key outputs are listed below. 

1. **Dissemination Block Urban/Rural Classification**:
   - **File**: `rurality-by-db.csv`
   - **Description**: Contains urban/rural classifications for each dissemination block, including area overlap proportions with population center boundaries.

2. **Service BC Facility Statistics**:
   - **File**: `sbc-location-statistics-for-SBC.csv`
   - **Description**: Comprehensive dataset for each Service BC facility, including population projections, age group breakdowns, drivetime metrics, distance bins, and time bins, and urban/rural classifications.
  
3. **Service BC Catchment Shapefiles**:  
   - **File**: `sbc-catchments/sbc-catchments.shp`  
   - **Description**: Geospatial shapefiles representing the catchment areas for each Service BC facility. These shapefiles are created by aggregating dissemination blocks (DBs) assigned to each facility and simplifying the geometries for efficient use in mapping and analysis.
...
The following R packages are required for the Service BC data pipeline:

- **Core Data Processing**: `tidyverse`, `glue`, `janitor`, `snakecase`.
- **Spatial Analysis**: `sf`, `bcdata`, `bcmaps`, `rmapshaper`, `tigris`, `spatstat`, `stars`, `terra`.
- **Visualization**: `ggplot2`, `scales`, `ggnewscale`.
- **Statistical Calculations**: `e1071`.
- **File Management**: `safepaths`, `fs`.

These packages are used for data cleaning, spatial analysis, visualization, and statistical calculations throughout the pipeline.
...
- **External Data**: Statistics Canada population center
