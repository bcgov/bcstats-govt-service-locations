...
is_in_region <- function(locations, regions, id_col, region_name_col) {
  ...
  message(glue("{n_matched} ({round(100*n_matched/nrow(results), 1)}%) of {id_col}'s were found within at least one {region_name_col} region."))
...
is_in_region_optim <- function(locations, regions, id_col, region_name_col, 
                                   area_threshold = 0.3) {
  ...
  single_intersect_cases <- intersection_candidates[
    intersection_candidates[[id_col]] %in% single_intersections, ]
  
  # Multiple intersections - need area calculations
  if (length(multiple_intersections) > 0) {
    multiple_candidates <- intersection_candidates[
      intersection_candidates[[id_col]] %in% multiple_intersections, ]
