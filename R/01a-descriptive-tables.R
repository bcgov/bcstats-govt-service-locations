...
#------------------------------------------------------------------------------
drivetime_stats_db <- calculate_drivetime_stats(
  drivetime_data,
  group_cols = c("csd_name", "csdid", "dbid")
)
drivetime_stats_csd <- calculate_drivetime_stats(
  drivetime_data,
  group_cols = c("csd_name", "csdid")
)
...
low_counts_prop <- sum(drivetime_stats_db$n_address < 5) /
  nrow(drivetime_stats_db)
message(glue(
...
low_counts <- drivetime_stats_db |>
  ...
  summarise(
    ...
    p_under_5_addresses = scales::percent(
      ifelse(n_db_blocks == 0, 0, n_under_5_addresses / n_db_blocks),
      accuracy = 0.1,
      trim = TRUE
    )
  ) |>
...
write_csv(
  drivetime_stats_csd,
  glue("{TABLES_OUT}/reduced_csd_average_times_dist_all_locs.csv")
)
write_csv(low_counts, glue("{TABLES_OUT}/reduced_csd_low_counts.csv"))
write_csv(
  servicebc_counts,
  glue("{TABLES_OUT}/reduced_csd_service_bc_counts.csv")
)
