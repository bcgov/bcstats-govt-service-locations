...
for (csd_name in unique(pilot_csds$csd_name)) {
  ...
  if (nrow(csd_facility) == 0) {
    warning(glue(
      "No Service BC facility found for {csd_name}. Skipping this CSD."
    ))
    next
  }
  ...
  if (nrow(facility_location) == 0) {
    warning(glue(
      "Could not find facility information for {facility_id}. Skipping this map."
    ))
    next
  }
  ...
  if (nrow(catchment_dbs) == 0) {
    warning(glue(
      "No DBs found in catchment for {facility_id}. Skipping this map."
    ))
    next
  }
  ...
  if (nrow(points) == 0) {
    warning(glue(
      "No points found in catchment for {facility_id}. Skipping this map."
    ))
    next
  }
  ...
  stats_ppp <- suppressWarnings(as.ppp(
    points$geometry,
    W = as.owin(catchment_shape)
  ))
  ...
  smooth_stats_stars <- tryCatch(
    ...
    error = function(e) {
      warning(glue(
        "Error generating spatial smooth map for {facility_id}: {e$message}"
      ))
      return(NULL)
    }
  )
  ...
  map_plot <- ggplot() +
    ...
    geom_sf(
      data = no_drive_data_dbs,
      aes(fill = "No Drive Data Available"),
      color = NA,
      alpha = 0.5
    ) +
    scale_fill_manual(
  ...
  fn <- to_snake_case(glue(
    "catchment-{plotvar}-commonscale={common_scale}-{csd_name}-{facility_id}"
  ))
  ggsave(
