...
if (nrow(shp_csd_all) == 0) {
  stop(
    "No matching census subdivisions found. Check the 'csd_name' values in the Service BC locations data."
  )
}
...
for (id in servicebc |> pull(csdid)) {
  ...
  if (nrow(points) == 0) {
    warning(glue(
      "No points found in census subdivision {csd}. Skipping this map."
    ))
    next
  }
  ...
  smooth_stats_stars <- tryCatch(
    ...
    error = function(e) {
      warning(glue(
        "Error generating spatial smooth map for {csd}: {e$message}"
      ))
      return(NULL)
    }
  )
  ...
  fn <- to_snake_case(glue(
    "drv-dist-smoothed-commonscale={common_scale}-{csd}"
  ))
